                ...
                <div class="card shadow-sm border-0">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <h5 class="fw-bold">${cita.servicio}</h5>
                            <span class="badge ${cita.estado === 'Confirmada' ? 'bg-success' : 'bg-info'}">${cita.estado}</span>
                        </div>
                        <p class="mb-1 text-muted">📅 ${cita.fecha}</p>
                        <p class="mb-0 text-muted">⏰ ${cita.hora_inicio} - ${cita.hora_fin}</p>
                        <hr>
                        <small class="text-secondary">Atendido por: ${cita.trabajador || 'Asignado automáticamente'}</small>
                    </div>
                </div>
            </div>
        `;
